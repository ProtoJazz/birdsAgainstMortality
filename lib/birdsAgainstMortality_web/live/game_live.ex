defmodule BirdsAgainstMortalityWeb.GameLive do
  ...
  def mount(params, session, socket) do
    ...
    {:ok, assign(socket, user: user, my_player: @default_player, open_chat: true)}
  end
  ...
  def start_game_server(game_id, deck_id \\ 1) do
    {:ok, _pid} =
      DynamicSupervisor.start_child(BirdsAgainstMortality.GameSupervisor, {Game, name: via_tuple(game_id), deck_id: deck_id})
  end
  ...
  defp draw_if_needed(%{assigns: %{game_id: game_id, user: user, game: game, state: state}} = socket) do
    ...
    if !is_nil(my_player) and  (Enum.count(my_player.hand) < game.hand_size) do
      ...
      :ok = GenServer.cast(via_tuple(game_id), {:deal, %{player: my_player, player_index: my_player_index, deal_count: (game.hand_size - Enum.count(my_player.hand))}})
  ...
  defp assign_game(%{assigns: %{game_id: game_id, user: user}} = socket) do
    ...
    socket
    |> assign(game: game, state: state, my_player: Enum.find(state.players, @default_player, fn player -> user.id == player.id end))
    |> draw_if_needed
  end
  ...
  def handle_event("change-name", %{"name" => name}, socket) do
    {:noreply,
     push_redirect(socket,
       to: Routes.page_path(socket, :index, %{new_name: name}), replace: true
     )}
  end
  ...
  def handle_event("play-card", %{"index" => index}, socket) do
     ...
     plays = if(!is_nil(socket.assigns.state.current_black_card)) do
      {parsed_plays, _} = Integer.parse(socket.assigns.state.current_black_card.play)
      parsed_plays
     else
      0
     end
      if (!is_nil(socket.assigns.my_player.id == socket.assigns.user.id) and !am_judge(socket) and (!is_nil(socket.assigns.state.current_black_card) and plays > Enum.count(socket.assigns.my_player.cards_in_play))) do
        {my_player, my_player_index} = Enum.find(Enum.with_index(game.state.players), nil, fn {l_player, _index} -> l_player.id == user.id end)
        :ok = GenServer.cast(via_tuple(socket.assigns.game_id), {:play_card, my_player, parsedIndex, my_player_index})
        :ok = Phoenix.PubSub.broadcast(BirdsAgainstMortality.PubSub, socket.assigns.game_id, :update)
      end
      {:noreply, socket}
  end
